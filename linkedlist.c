...
void ll_insert_back(linked_list* list, void* item, size_t datasize)
{
	...
	else
	{
		...
		for (unsigned int i = 0; i < ll_get_length(*list) - 1; i++)
...
{
	return *ll_get_item_ptr(list, index);
}
...
void ll_free_list(linked_list* list)
{
	...
	while ((length = ll_get_length(*list)) > 1)
	{
		//get the last two items
		ll_node* free_item = ll_get_item_ptr(*list, length - 1);
		ll_node* prior_item = ll_get_item_ptr(*list, length - 2);
